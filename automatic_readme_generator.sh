...
do
    ...
    then
        ...
        _notes=`echo -e "$_notes \`git log $_first_commit --oneline | grep \"Commit of\"\`"`
        _notes=`echo -e "${tags[$_index_tags]}"`
      
    else
        _notes=`echo -e "$_notes \`git log ${tags[$((_index_tags-1))]}..${tags[$((_index_tags))]} --oneline | grep "Commit of"\`"`
